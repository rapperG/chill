...
import com.chill.token.config.ChillSignConfig;
import com.chill.token.context.model.ChillRequest;
import com.chill.token.error.ChillErrorCode;
import com.chill.token.exception.SignException;
import com.chill.token.secure.ChillSecureUtil;
 ...
 * <p>
 * 以 SSO 数据拉取为例，流程大致如下：
 * <br> 1. 以 md5( loginId={账号id}&nonce={随机字符串}&timestamp={13位时间戳}&key={secretkey秘钥} ) 生成签名 sign。
 * <br> 2. 将 sign 作为参数，拼接到请求地址后面，如：http://xxx.com?loginId=10001 & nonce=xxx & timestamp=xxx & sign=xxx。
 ...
 * @since 1.0
 */
public class ChillSignTemplate {

    // ----------- 签名配置

    ChillSignConfig signConfig;
     ...
     * @return /
     */
    public ChillSignConfig getSignConfig() {
        return signConfig;
    }
     ...
     * @return /
     */
    public ChillSignConfig getSignConfigOrGlobal() {
        // 如果用户自定义了 signConfig ，则使用用户自定义的
        if (signConfig != null) {
            return signConfig;
        }
        // 否则使用全局默认配置
        return ChillManager.getConfig().getSign();
    }
     ...
     * @param signConfig /
     */
    public ChillSignTemplate setSignConfig(ChillSignConfig signConfig) {
    ...
    public String createSign(Map<String, ?> paramsMap) {
        ...
        SignException.throwByNull(secretKey, "参与参数签名的秘钥不可为空", ChillErrorCode.CODE_12201);
    ...
    public String abstractStr(String fullStr) {
        return ChillSecureUtil.md5(fullStr);
    }
    ...
    public Map<String, Object> addSignParams(Map<String, Object> paramsMap) {
        ...
        paramsMap.put(nonce, ChillFoxUtil.getRandomString(32));
    ...
    public void checkTimestamp(long timestamp) {
        if (!isValidTimestamp(timestamp)) {
            throw new SignException("timestamp 超出允许的范围：" + timestamp).setCode(ChillErrorCode.CODE_12203);
        }
    }
    ...
    public boolean isValidNonce(String nonce) {
        // 为空代表无效
        if (ChillFoxUtil.isEmpty(nonce)) {
            return false;
        }

        // 校验此 nonce 是否已被使用过
        String key = splicingNonceSaveKey(nonce);
        return ChillManager.getChillTokenDao().get(key) == null;
    }
    ...
    public void checkNonce(String nonce) {
        // 为空代表无效
        if (ChillFoxUtil.isEmpty(nonce)) {
            throw new SignException("nonce 为空，无效");
        }

        // 校验此 nonce 是否已被使用过
        String key = splicingNonceSaveKey(nonce);
        if (ChillManager.getChillTokenDao().get(key) != null) {
            throw new SignException("此 nonce 已被使用过，不可重复使用：" + nonce);
        }

        // 校验通过后，将此 nonce 保存在缓存中，保证下次校验无法通过
        ChillManager.getChillTokenDao().set(key, nonce, getSignConfigOrGlobal().getSaveNonceExpire() * 2 + 2);
    }
    ...
    public void checkSign(Map<String, ?> paramsMap, String sign) {
        if (!isValidSign(paramsMap, sign)) {
            throw new SignException("无效签名：" + sign).setCode(ChillErrorCode.CODE_12202);
        }
    }
    ...
    public boolean isValidParamMap(Map<String, String> paramMap) {
        ...
        // 三个参数必须全部非空
        SignException.throwByNull(timestampValue, "缺少 timestamp 字段");
        SignException.throwByNull(nonceValue, "缺少 nonce 字段");
        SignException.throwByNull(signValue, "缺少 sign 字段");
     ...
     * @return 是否合法
     */
    public boolean isValidRequest(ChillRequest request) {
     ...
     * @param request 待校验的请求对象
     */
    public void checkRequest(ChillRequest request) {
    ...
    public String splicingNonceSaveKey(String nonce) {
        return ChillManager.getConfig().getTokenName() + ":sign:nonce:" + nonce;
    }

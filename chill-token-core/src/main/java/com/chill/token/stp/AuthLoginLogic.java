...
import com.chill.token.config.ChillCookieConfig;
import com.chill.token.config.ChillTokenConfig;
import com.chill.token.context.ChillHolder;
import com.chill.token.context.model.ChillCookie;
...
import com.chill.token.exception.*;
...
import com.chill.token.session.ChillTokenSign;
...
import com.chill.token.util.ChillValue2Box;
...
import static com.chill.token.exception.NotLoginException.*;


/**
 * Chill-Token 权限认证，逻辑实现类
 *
 * <p>
 * Chill-Token 的核心，框架大多数功能均由此类提供具体逻辑实现。
 * </p>
 *
 * @author chill
 * @since 1.0
 */
public class AuthLoginLogic {
    ...
    /**
     * 初始化 AuthLoginLogic, 并指定账号类型
     *
     * @param loginType 账号类型标识
     */
    public AuthLoginLogic(String loginType) {
        setLoginType(loginType);
    }

    /**
     * 获取当前 AuthLoginLogic 账号类型标识
     *
     * @return /
     */
     ...
     * @return 对象自身
     */
    public AuthLoginLogic setLoginType(String loginType) {

        // 先清除此 AuthLoginLogic 在全局 ChillManager 中的记录
        if (ChillFoxUtil.isNotEmpty(this.loginType)) {
            ChillManager.removeAuthLoginLogicComponent(this.loginType);
        }

        // 赋值
        this.loginType = loginType;

        // 将新的 loginType -> AuthLoginLogic 映射关系 put 到 ChillManager 全局集合中，以便后续根据 LoginType 进行查找此对象
        ChillManager.putAuthLoginLogicComponent(this);

        return this;
    }

    private ChillTokenConfig config;

    /**
     * 写入当前 AuthLoginLogic 单独使用的配置对象
     *
     * @param config 配置对象
     * @return 对象自身
     */
    public AuthLoginLogic setConfig(ChillTokenConfig config) {
        this.config = config;
        return this;
    }

    /**
     * 返回当前 AuthLoginLogic 使用的配置对象，如果当前 AuthLoginLogic 没有配置，则返回 null
     *
     * @return /
     */
    public ChillTokenConfig getConfig() {
        return config;
    }

    /**
     * 返回当前 AuthLoginLogic 使用的配置对象，如果当前 AuthLoginLogic 没有配置，则返回全局配置对象
     *
     * @return /
     */
    public ChillTokenConfig getConfigOrGlobal() {
        ChillTokenConfig cfg = getConfig();
        if (cfg != null) {
            return cfg;
        }
        return ChillManager.getConfig();
    }
    ...
    public String createTokenValue(Object loginId, String device, long timeout, Map<String, Object> extraData) {
        return ChillStrategy.INSTANCE.createToken.apply(loginId, loginType);
    }
    ...
    public void setTokenValue(String tokenValue) {
        setTokenValue(tokenValue, new ChillLoginModel().setTimeout(getConfigOrGlobal().getTimeout()));
    }
    ...
    public void setTokenValue(String tokenValue, int cookieTimeout) {
        setTokenValue(tokenValue, new ChillLoginModel().setTimeout(cookieTimeout));
    }
     ...
     */
    public void setTokenValue(String tokenValue, ChillLoginModel loginModel) {

        // 先判断一下，如果提供 token 为空，则不执行任何动作
        if (ChillFoxUtil.isEmpty(tokenValue)) {
            return;
        }
    ...
    public void setTokenValueToCookie(String tokenValue, int cookieTimeout) {
        ChillCookieConfig cfg = getConfigOrGlobal().getCookie();
        ChillCookie cookie = new ChillCookie()
        ...
        ChillHolder.getResponse().addCookie(cookie);
    }
    ...
    public void setTokenValueToResponseHeader(String tokenValue) {
        ...
        ChillResponse response = ChillHolder.getResponse();
        response.setHeader(tokenName, tokenValue);

        // 此处必须在响应头里指定 Access-Control-Expose-Headers: token-name，否则前端无法读取到这个响应头
        response.addHeader(ChillResponse.ACCESS_CONTROL_EXPOSE_HEADERS, tokenName);
    }
    ...
    public String getTokenValue(boolean noPrefixThrowException) {
        ...
        String tokenPrefix = getConfigOrGlobal().getTokenPrefix();
        if (ChillFoxUtil.isNotEmpty(tokenPrefix)) {
            ...
            else if (!tokenValue.startsWith(tokenPrefix + ChillTokenConstants.TOKEN_CONNECTOR_CHAT)) {
                if (noPrefixThrowException) {
                    throw NotLoginException.newInstance(loginType, NO_PREFIX, NO_PREFIX_MESSAGE + "，prefix=" + tokenPrefix, null).setCode(ChillErrorCode.CODE_11017);
                } else {
                    tokenValue = null;
                }
            }
            ...
            // 情况2.3：代码至此，说明 token 有值，且是以指定的前缀开头的，现在裁剪掉前缀
                ...
                tokenValue = tokenValue.substring(tokenPrefix.length() + ChillTokenConstants.TOKEN_CONNECTOR_CHAT.length());
            }
        }
    ...
    public String getTokenValueNotCut() {
        ...
        // 获取相应对象
        ChillStorage storage = ChillHolder.getStorage();
        ChillRequest request = ChillHolder.getRequest();
        ChillTokenConfig config = getConfigOrGlobal();
    ...
    public String getTokenValueNotNull() {
        ...
        if (ChillFoxUtil.isEmpty(tokenValue)) {
            throw NotLoginException.newInstance(loginType, NOT_TOKEN, NOT_TOKEN_MESSAGE, null).setCode(ChillErrorCode.CODE_11001);
        }
        return tokenValue;
    }
     ...
     * @return token 参数信息
     */
    public ChillTokenInfo getTokenInfo() {
        ChillTokenInfo info = new ChillTokenInfo();
    ...
    public void login(Object id, String device) {
        login(id, new ChillLoginModel().setDevice(device));
    }
    ...
    public void login(Object id, boolean isLastingCookie) {
        login(id, new ChillLoginModel().setIsLastingCookie(isLastingCookie));
    }
    ...
    public void login(Object id, long timeout) {
        login(id, new ChillLoginModel().setTimeout(timeout));
    }
     ...
     */
    public void login(Object id, ChillLoginModel loginModel) {
        // 1、创建会话
    ...
    public String createLoginSession(Object id) {
        return createLoginSession(id, new ChillLoginModel());
    }
     ...
     * @return 返回会话令牌
     */
    public String createLoginSession(Object id, ChillLoginModel loginModel) {
        ...
        ChillSession session = getSessionByLoginId(id, true);
        session.updateMinTimeout(loginModel.getTimeout());

        // 5、在 Account-Session 上记录本次登录的 token 签名
        ChillTokenSign tokenSign = new ChillTokenSign(tokenValue, loginModel.getDeviceOrDefault(), loginModel.getTokenSignTag());
        ...
        ChillTokenEventCenter.doLogin(loginType, id, tokenValue, loginModel);
     ...
     * @return 返回 token
     */
    protected String distUsableToken(Object id, ChillLoginModel loginModel) {
        ...
        if (ChillFoxUtil.isNotEmpty(loginModel.getToken())) {
        ...
        if (isConcurrent) {
            ...
            if (getConfigOfIsShare()) {
                ...
                // 如果有值，那就直接复用
                if (ChillFoxUtil.isNotEmpty(tokenValue)) {
                    return tokenValue;
                }
     ...
     * @param loginModel 此次登录的参数Model
     */
    protected void checkLoginArgs(Object id, ChillLoginModel loginModel) {

        // 1、账号 id 不能为空
        if (ChillFoxUtil.isEmpty(id)) {
            throw new TokenException("loginId 不能为空").setCode(ChillErrorCode.CODE_11002);
        }

        // 2、账号 id 不能是异常标记值
        if (NotLoginException.ABNORMAL_LIST.contains(id.toString())) {
            throw new TokenException("loginId 不能为以下值：" + NotLoginException.ABNORMAL_LIST);
        }

        // 3、账号 id 不能是简单类型
        if (!ChillFoxUtil.isBasicType(id.getClass())) {
            ChillManager.log.warn("loginId 应该为简单类型，例如：String | int | long，不推荐使用复杂类型：" + id.getClass());
        }

        // 4、判断当前 AuthLoginLogic 是否支持 extra 扩展参数
        if (!isSupportExtra()) {
            // 如果不支持，开发者却传入了 extra 扩展参数，那么就打印警告信息
            Map<String, Object> extraData = loginModel.getExtraData();
            if (extraData != null && extraData.size() > 0) {
                ChillManager.log.warn("当前 AuthLoginLogic 不支持 extra 扩展参数模式，传入的 extra 参数将被忽略");
            }
        }
        ...
        if (!getConfigOrGlobal().getDynamicActiveTimeout() && loginModel.getActiveTimeout() != null) {
            ChillManager.log.warn("当前全局配置未开启动态 activeTimeout 功能，传入的 activeTimeout 参数将被忽略");
        }
    ...
    public void logout() {
        ...
        storage.delete(ChillTokenConstants.TOKEN_ACTIVE_TIMEOUT_CHECKED_KEY);
    ...
    public void logout(Object loginId, String device) {
        ...
        ChillSession session = getSessionByLoginId(loginId, false);
        if (session != null) {

            // 2、遍历此账号所有从这个 device 设备上登录的客户端，清除相关数据
            for (ChillTokenSign tokenSign : session.getTokenSignListByDevice(device)) {
                ...
                ChillTokenEventCenter.doLogout(loginType, loginId, tokenValue);
            }
     ...
     */
    public void logoutByMaxLoginCount(Object loginId, ChillSession session, String device, int maxLoginCount) {
        ...
        List<ChillTokenSign> list = session.getTokenSignListByDevice(device);
    ...
    public void kickout(Object loginId, String device) {
        ...
        if (session != null) {
            ...
            for (ChillTokenSign tokenSign : session.getTokenSignListByDevice(device)) {
                ...
                ChillTokenEventCenter.doKickout(loginType, loginId, tokenValue);
            }
    ...
    public void replaced(Object loginId, String device) {
        ...
        if (session != null) {
            ...
            for (ChillTokenSign tokenSign : session.getTokenSignListByDevice(device)) {
                ...
                // 2.6、$$ 发布事件：xx 账号的 xx 客户端注销了
                ChillTokenEventCenter.doReplaced(loginType, loginId, tokenValue);
            }
    ...
    public Object getLoginId() {
        ...
        String tokenValue = getTokenValue(true);
        if (ChillFoxUtil.isEmpty(tokenValue)) {
            throw NotLoginException.newInstance(loginType, NOT_TOKEN, NOT_TOKEN_MESSAGE, null).setCode(ChillErrorCode.CODE_11011);
        }

        // 3、查找此 token 对应的 loginId，如果找不到则抛出：token 无效
        String loginId = getLoginIdNotHandle(tokenValue);
        if (ChillFoxUtil.isEmpty(loginId)) {
            throw NotLoginException.newInstance(loginType, INVALID_TOKEN, INVALID_TOKEN_MESSAGE, tokenValue).setCode(ChillErrorCode.CODE_11012);
        }

        // 4、如果这个 token 指向的是值是：过期标记，则抛出：token 已过期
        if (loginId.equals(NotLoginException.TOKEN_TIMEOUT)) {
            throw NotLoginException.newInstance(loginType, TOKEN_TIMEOUT, TOKEN_TIMEOUT_MESSAGE, tokenValue).setCode(ChillErrorCode.CODE_11013);
        }

        // 5、如果这个 token 指向的是值是：被顶替标记，则抛出：token 已被顶下线
        if (loginId.equals(NotLoginException.BE_REPLACED)) {
            throw NotLoginException.newInstance(loginType, BE_REPLACED, BE_REPLACED_MESSAGE, tokenValue).setCode(ChillErrorCode.CODE_11014);
        }

        // 6、如果这个 token 指向的是值是：被踢下线标记，则抛出：token 已被踢下线
        if (loginId.equals(NotLoginException.KICK_OUT)) {
            throw NotLoginException.newInstance(loginType, KICK_OUT, KICK_OUT_MESSAGE, tokenValue).setCode(ChillErrorCode.CODE_11015);
        }
    ...
    public <T> T getLoginId(T defaultValue) {
        ...
        // 3、不为 null，则开始尝试类型转换
        return (T) ChillFoxUtil.getValueByType(loginId, defaultValue.getClass());
    }
    ...
    public Object getLoginIdDefaultNull() {
        ...
        if (getTokenActiveTimeoutByToken(tokenValue) == ChillTokenDao.NOT_VALUE_EXPIRE) {
            return null;
        }
    ...
    public Object getExtra(String key) {
        throw new ApiDisabledException("只有在集成 Chill-Token-jwt 插件后才可以使用 extra 扩展参数").setCode(ChillErrorCode.CODE_11031);
    }
    ...
    public boolean isValidLoginId(Object loginId) {
        ...
        return ChillFoxUtil.isNotEmpty(loginId) && !NotLoginException.ABNORMAL_LIST.contains(loginId.toString());
    }
    ...
    public void updateTokenToIdMapping(String tokenValue, Object loginId) {
        // 先判断一下，是否传入了空值
        TokenException.throwBy(ChillFoxUtil.isEmpty(loginId), "loginId 不能为空", ChillErrorCode.CODE_11003);
    ...
    /**
     * 获取指定 key 的 ChillSession, 如果该 ChillSession 尚未创建，isCreate = 是否立即新建并返回
     *
     * @param sessionId       SessionId
     * @param isCreate        是否新建
     * @param appendOperation 如果这个 ChillSession 是新建的，则要追加执行的动作
     * @return Session对象
     */
    public ChillSession getSessionBySessionId(String sessionId, boolean isCreate, Consumer<ChillSession> appendOperation) {

        // 先检查这个 ChillSession 是否已经存在，如果不存在且 isCreate=true，则新建并返回
        ChillSession session = getSaTokenDao().getSession(sessionId);

        if (session == null && isCreate) {
            // 创建这个 ChillSession
            session = ChillStrategy.INSTANCE.createSession.apply(sessionId);
    ...
    /**
     * 获取指定 key 的 ChillSession, 如果该 ChillSession 尚未创建，则返回 null
     *
     * @param sessionId SessionId
     * @return Session对象
     */
    public ChillSession getSessionBySessionId(String sessionId) {
        return getSessionBySessionId(sessionId, false, null);
    }

    /**
     * 获取指定账号 id 的 Account-Session, 如果该 ChillSession 尚未创建，isCreate=是否新建并返回
     *
     * @param loginId  账号id
     * @param isCreate 是否新建
     * @return ChillSession 对象
     */
    public ChillSession getSessionByLoginId(Object loginId, boolean isCreate) {
        return getSessionBySessionId(splicingKeySession(loginId), isCreate, session -> {
            // 这里是该 Account-Session 首次创建时才会被执行的方法：
            // 		设定这个 ChillSession 的各种基础信息：类型、账号体系、账号id
            session.setType(ChillTokenConstants.SESSION_TYPE__ACCOUNT);
    ...
    /**
     * 获取指定账号 id 的 Account-Session，如果该 ChillSession 尚未创建，则新建并返回
     *
     * @param loginId 账号id
     * @return ChillSession 对象
     */
    public ChillSession getSessionByLoginId(Object loginId) {
        return getSessionByLoginId(loginId, true);
    }

    /**
     * 获取当前已登录账号的 Account-Session, 如果该 ChillSession 尚未创建，isCreate=是否新建并返回
     *
     * @param isCreate 是否新建
     * @return Session对象
     */
    public ChillSession getSession(boolean isCreate) {
        return getSessionByLoginId(getLoginId(), isCreate);
    }

    /**
     * 获取当前已登录账号的 Account-Session，如果该 ChillSession 尚未创建，则新建并返回
     *
     * @return Session对象
     */
    public ChillSession getSession() {
    ...
    /**
     * 获取指定 token 的 Token-Session，如果该 ChillSession 尚未创建，isCreate代表是否新建并返回
     *
     * @param tokenValue token值
     * @param isCreate   是否新建
     * @return session对象
     */
    public ChillSession getTokenSessionByToken(String tokenValue, boolean isCreate) {
        return getSessionBySessionId(splicingKeyTokenSession(tokenValue), isCreate, session -> {
            // 这里是该 Token-Session 首次创建时才会被执行的方法：
            // 		设定这个 ChillSession 的各种基础信息：类型、账号体系、Token 值
            session.setType(ChillTokenConstants.SESSION_TYPE__TOKEN);
            session.setLoginType(getLoginType());
    ...
    /**
     * 获取指定 token 的 Token-Session，如果该 ChillSession 尚未创建，则新建并返回
     *
     * @param tokenValue Token值
     * @return Session对象
     */
    public ChillSession getTokenSessionByToken(String tokenValue) {
        return getTokenSessionByToken(tokenValue, true);
    }

    /**
     * 获取当前 token 的 Token-Session，如果该 ChillSession 尚未创建，isCreate代表是否新建并返回
     *
     * @param isCreate 是否新建
     * @return Session对象
     */
    public ChillSession getTokenSession(boolean isCreate) {
        ...
        //    现在根据前端提供的 Token 获取它对应的 Token-Session 对象（ChillSession）
        return getTokenSessionByToken(tokenValue, isCreate);
    }

    /**
     * 获取当前 token 的 Token-Session，如果该 ChillSession 尚未创建，则新建并返回
     *
     * @return Session对象
     */
    public ChillSession getTokenSession() {
     ...
     */
    public ChillSession getAnonTokenSession(boolean isCreate) {
        /*
        ...
        if (ChillFoxUtil.isNotEmpty(tokenValue)) {
            ...
            // 符合条件1
            ChillSession session = getTokenSessionByToken(tokenValue, false);
            if (session != null) {
                return session;
            }
        ...
        // q2 —— 此时q2分两种情况：
        /*
         * 情况 2.1、isCreate=true：说明调用方想让框架帮其创建一个 ChillSession，那框架就创建并返回
         * 情况 2.2、isCreate=false：说明调用方并不想让框架帮其创建一个 ChillSession，那框架就直接返回 null
         */
        if (isCreate) {
            // 随机创建一个 Token
            tokenValue = ChillStrategy.INSTANCE.generateUniqueToken.execute(
                "token",
                getConfigOfMaxTryTimes(),
                () -> {
     ...
     * @return Token-Session 对象
     */
    public ChillSession getAnonTokenSession() {
    ...
    public void updateLastActiveToNow(String tokenValue) {
        String key = splicingKeyLastActiveTime(tokenValue);
        String value = new ChillValue2Box(System.currentTimeMillis(), getTokenUseActiveTimeout(tokenValue)).toString();
    ...
    public void checkActiveTimeout(String tokenValue) {
        ...
        ChillStorage storage = ChillHolder.getStorage();
        storage.get(ChillTokenConstants.TOKEN_ACTIVE_TIMEOUT_CHECKED_KEY, () -> {

            // 1、获取这个 token 的剩余活跃有效期
            long activeTimeout = getTokenActiveTimeoutByToken(tokenValue);

            // 2、值为 -1 代表此 token 已经被设置永不冻结，无须继续验证
            if (activeTimeout == ChillTokenDao.NEVER_EXPIRE) {
                return true;
            }

            // 3、值为 -2 代表已被冻结，此时需要抛出异常
            if (activeTimeout == ChillTokenDao.NOT_VALUE_EXPIRE) {
                throw NotLoginException.newInstance(loginType, TOKEN_FREEZE, TOKEN_FREEZE_MESSAGE, tokenValue).setCode(ChillErrorCode.CODE_11016);
            }
    ...
    public Long getTokenUseActiveTimeout(String tokenValue) {
        ...
        // 解析，无值的情况下返回 null
        ChillValue2Box box = new ChillValue2Box(value);
    ...
    public long getTokenActiveTimeoutByToken(String tokenValue) {
        ...
        // 如果全局配置了永不冻结, 则返回 -1
        if (!isOpenCheckActiveTimeout()) {
            return ChillTokenDao.NEVER_EXPIRE;
        }

        // 如果提供的 token 为 null，则返回 -2
        if (ChillFoxUtil.isEmpty(tokenValue)) {
            return ChillTokenDao.NOT_VALUE_EXPIRE;
        }
        ...
        ChillValue2Box box = new ChillValue2Box(lastActiveTimeString);
        ...
        if (allowTimeDiff == ChillTokenDao.NEVER_EXPIRE) {
    ...
    public void renewTimeout(long timeout) {
        ...
        if (getConfigOrGlobal().getIsReadCookie()) {
            ...
            if (timeout == ChillTokenDao.NEVER_EXPIRE || timeout > Integer.MAX_VALUE) {
    ...
    public void renewTimeout(String tokenValue, long timeout) {
        ...
        ChillTokenDao dao = getSaTokenDao();
        dao.updateTimeout(splicingKeyTokenValue(tokenValue), timeout);

        // 3、续期此 token 的 Token-Session 有效期
        ChillSession tokenSession = getTokenSessionByToken(tokenValue, false);
        ...
        ChillTokenEventCenter.doRenewTimeout(tokenValue, loginId, timeout);
    }
    ...
    public List<String> getRoleList(Object loginId) {
        return ChillManager.getAuthDataInterface().roles(loginId, loginType);
    }
    ...
    public void checkRole(String role) {
        if (!hasRole(role)) {
            throw new NotRoleException(role, this.loginType).setCode(ChillErrorCode.CODE_11041);
        }
    }
    ...
    public void checkRoleOr(String... roleArray) {
        ...
        throw new NotRoleException(roleArray[0], this.loginType).setCode(ChillErrorCode.CODE_11041);
    }
    ...
    public List<String> getPermissionList(Object loginId) {
        return ChillManager.getAuthDataInterface().permissions(loginId, loginType);
    }
    ...
    public void checkPermission(String permission) {
        if (!hasPermission(permission)) {
            throw new NotPermissionException(permission, this.loginType).setCode(ChillErrorCode.CODE_11051);
        }
    }
    ...
    public void checkPermissionOr(String... permissionArray) {
        ...
        throw new NotPermissionException(permissionArray[0], this.loginType).setCode(ChillErrorCode.CODE_11051);
    }
     ...
     * @return 此 loginId 的所有登录 token
     */
    public List<ChillTokenSign> getTokenSignListByLoginId(Object loginId, String device) {
        // 如果该账号的 Account-Session 为 null，说明此账号尚没有客户端在登录，此时返回空集合
        ChillSession session = getSessionByLoginId(loginId, false);
        if (session == null) {
            return Collections.emptyList();
    ...
    public String getLoginDevice() {
        ...
        ChillSession session = getSessionByLoginId(getLoginIdDefaultNull(), false);
    ...
    /**
     * 根据注解 ( @ChillCheckLogin ) 鉴权
     *
     * @param at 注解对象
     */
    public void checkByAnnotation(ChillCheckLogin at) {
        this.checkLogin();
    }

    /**
     * 根据注解 ( @ChillCheckRole ) 鉴权
     *
     * @param at 注解对象
     */
    public void checkByAnnotation(ChillCheckRole at) {
        String[] roleArray = at.value();
        if (at.mode() == ChillCheckMode.AND) {
    ...
    /**
     * 根据注解 ( @ChillCheckPermission ) 鉴权
     *
     * @param at 注解对象
     */
    public void checkByAnnotation(ChillCheckPermission at) {
        ...
        try {
            ...
            for (String role : at.orRole()) {
                String[] rArr = ChillFoxUtil.convertStringToArray(role);
    ...
    /**
     * 根据注解 ( @ChillCheckSafe ) 鉴权
     *
     * @param at 注解对象
     */
    public void checkByAnnotation(ChillCheckSafe at) {
        this.checkSafe(at.value());
    }

    /**
     * 根据注解 ( @ChillCheckDisable ) 鉴权
     *
     * @param at 注解对象
     */
    public void checkByAnnotation(ChillCheckDisable at) {
        Object loginId = getLoginId();
    ...
    /**
     * 封禁：指定账号
     * <p> 此方法不会直接将此账号id踢下线，如需封禁后立即掉线，请追加调用 AuthLoginUtil.logout(id)
     *
     * @param loginId 指定账号id
     * @param time    封禁时间, 单位: 秒 （-1=永久封禁）
     */
    public void disable(Object loginId, long time) {
        disableLevel(loginId, ChillTokenConstants.DEFAULT_DISABLE_SERVICE, ChillTokenConstants.DEFAULT_DISABLE_LEVEL, time);
    }
    ...
    public boolean isDisable(Object loginId) {
        return isDisableLevel(loginId, ChillTokenConstants.DEFAULT_DISABLE_SERVICE, ChillTokenConstants.MIN_DISABLE_LEVEL);
    }
    ...
    public void checkDisable(Object loginId) {
        checkDisableLevel(loginId, ChillTokenConstants.DEFAULT_DISABLE_SERVICE, ChillTokenConstants.MIN_DISABLE_LEVEL);
    }
    ...
    public long getDisableTime(Object loginId) {
        return getDisableTime(loginId, ChillTokenConstants.DEFAULT_DISABLE_SERVICE);
    }
    ...
    public void untieDisable(Object loginId) {
        untieDisable(loginId, ChillTokenConstants.DEFAULT_DISABLE_SERVICE);
    }
    ...
    public void disable(Object loginId, String service, long time) {
        disableLevel(loginId, service, ChillTokenConstants.DEFAULT_DISABLE_LEVEL, time);
    }
    ...
    public boolean isDisable(Object loginId, String service) {
        return isDisableLevel(loginId, service, ChillTokenConstants.MIN_DISABLE_LEVEL);
    }
    ...
    public void checkDisable(Object loginId, String... services) {
        if (services != null) {
            for (String service : services) {
                checkDisableLevel(loginId, service, ChillTokenConstants.MIN_DISABLE_LEVEL);
            }
        }
    }
    ...
    public void untieDisable(Object loginId, String... services) {
        ...
        // 先检查提供的参数是否有效
        if (ChillFoxUtil.isEmpty(loginId)) {
            throw new TokenException("请提供要解禁的账号").setCode(ChillErrorCode.CODE_11062);
        }
        if (services == null || services.length == 0) {
            throw new TokenException("请提供要解禁的服务").setCode(ChillErrorCode.CODE_11063);
        }
        ...
        for (String service : services) {
            ...
            // $$ 发布事件
            ChillTokenEventCenter.doUntieDisable(loginType, loginId, service);
        }
    }
    ...
    public void disableLevel(Object loginId, int level, long time) {
        disableLevel(loginId, ChillTokenConstants.DEFAULT_DISABLE_SERVICE, level, time);
    }
    ...
    public void disableLevel(Object loginId, String service, int level, long time) {
        // 先检查提供的参数是否有效
        if (ChillFoxUtil.isEmpty(loginId)) {
            throw new TokenException("请提供要封禁的账号").setCode(ChillErrorCode.CODE_11062);
        }
        if (ChillFoxUtil.isEmpty(service)) {
            throw new TokenException("请提供要封禁的服务").setCode(ChillErrorCode.CODE_11063);
        }
        if (level < ChillTokenConstants.MIN_DISABLE_LEVEL) {
            throw new TokenException("封禁等级不可以小于最小值：" + ChillTokenConstants.MIN_DISABLE_LEVEL).setCode(ChillErrorCode.CODE_11064);
        }
        ...
        // $$ 发布事件
        ChillTokenEventCenter.doDisable(loginType, loginId, service, level, time);
    }
    ...
    public boolean isDisableLevel(Object loginId, int level) {
        return isDisableLevel(loginId, ChillTokenConstants.DEFAULT_DISABLE_SERVICE, level);
    }
    ...
    public boolean isDisableLevel(Object loginId, String service, int level) {
        ...
        if (disableLevel == ChillTokenConstants.NOT_DISABLE_LEVEL) {
            return false;
        }
    ...
    public void checkDisableLevel(Object loginId, int level) {
        checkDisableLevel(loginId, ChillTokenConstants.DEFAULT_DISABLE_SERVICE, level);
    }
    ...
    public void checkDisableLevel(Object loginId, String service, int level) {
        ...
        // 2、再判断被封禁的等级是否达到了指定级别
        Integer disableLevel = ChillFoxUtil.getValueByType(value, int.class);
        if (disableLevel >= level) {
            throw new DisableServiceException(loginType, loginId, service, disableLevel, level, getDisableTime(loginId, service))
                .setCode(ChillErrorCode.CODE_11061);
        }
    }
    ...
    public int getDisableLevel(Object loginId) {
        return getDisableLevel(loginId, ChillTokenConstants.DEFAULT_DISABLE_SERVICE);
    }
    ...
    public int getDisableLevel(Object loginId, String service) {
        ...
        String value = getSaTokenDao().get(splicingKeyDisable(loginId, service));
        if (ChillFoxUtil.isEmpty(value)) {
            return ChillTokenConstants.NOT_DISABLE_LEVEL;
        }

        // 2、转为 int 类型返回
        return ChillFoxUtil.getValueByType(value, int.class);
    }
    ...
    public void switchTo(Object loginId) {
        ChillHolder.getStorage().set(splicingKeySwitch(), loginId);
    }
    ...
    public void endSwitch() {
        ChillHolder.getStorage().delete(splicingKeySwitch());
    }
    ...
    public boolean isSwitch() {
        return ChillHolder.getStorage().get(splicingKeySwitch()) != null;
    }
    ...
    public Object getSwitchLoginId() {
        return ChillHolder.getStorage().get(splicingKeySwitch());
    }
     ...
     */
    public void switchTo(Object loginId, ChillFunction function) {
        try {
            switchTo(loginId);
            function.run();
        } finally {
    ...
    public void openSafe(long safeTime) {
        openSafe(ChillTokenConstants.DEFAULT_SAFE_AUTH_SERVICE, safeTime);
    }
    ...
    public void openSafe(String service, long safeTime) {
        ...
        getSaTokenDao().set(splicingKeySafe(tokenValue, service), ChillTokenConstants.SAFE_AUTH_SAVE_VALUE, safeTime);

        // 3、$$ 发布事件，某某 token 令牌开启了二级认证
        ChillTokenEventCenter.doOpenSafe(loginType, tokenValue, service, safeTime);
    }
    ...
    public boolean isSafe() {
        return isSafe(ChillTokenConstants.DEFAULT_SAFE_AUTH_SERVICE);
    }
    ...
    public boolean isSafe(String tokenValue, String service) {
        ...
        return !(ChillFoxUtil.isEmpty(value));
    }
    ...
    public void checkSafe() {
        checkSafe(ChillTokenConstants.DEFAULT_SAFE_AUTH_SERVICE);
    }
    ...
    public void checkSafe(String service) {
        ...
        if (!isSafe(tokenValue, service)) {
            throw new NotSafeException(loginType, tokenValue, service).setCode(ChillErrorCode.CODE_11071);
        }
    }
    ...
    public long getSafeTime() {
        return getSafeTime(ChillTokenConstants.DEFAULT_SAFE_AUTH_SERVICE);
    }
    ...
    public void closeSafe() {
        closeSafe(ChillTokenConstants.DEFAULT_SAFE_AUTH_SERVICE);
    }
    ...
    public void closeSafe(String service) {
        ...
        ChillTokenEventCenter.doCloseSafe(loginType, tokenValue, service);
    }
    ...
    public String splicingKeySwitch() {
        return ChillTokenConstants.SWITCH_TO_SAVE_KEY + loginType;
    }
...
//		return ChillTokenConstants.JUST_CREATED_SAVE_KEY + loginType;
        return ChillTokenConstants.JUST_CREATED;
    }
    ...
    /**
     * 返回当前 AuthLoginLogic 使用的持久化对象
     *
     * @return /
     */
    public ChillTokenDao getSaTokenDao() {
        return ChillManager.getChillTokenDao();
    }
    ...
    public boolean isOpenCheckActiveTimeout() {
        ChillTokenConfig cfg = getConfigOrGlobal();
        return cfg.getActiveTimeout() != ChillTokenDao.NEVER_EXPIRE || cfg.getDynamicActiveTimeout();
    }
    ...
    public int getConfigOfCookieTimeout() {
        ...
        if (timeout == ChillTokenDao.NEVER_EXPIRE) {
    ...
    public boolean hasElement(List<String> list, String element) {
        return ChillStrategy.INSTANCE.hasElement.apply(list, element);
    }

    /**
     * 当前 AuthLoginLogic 对象是否支持 token 扩展参数
     *
     * @return /
     */

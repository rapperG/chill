...
import static com.chill.token.ChillManager.log;

/**
 * Chill-Token 侦听器的一个实现：Log 打印
 *
 * @author chill
 * @since 1.0
 */
public class ChillTokenListenerForLog implements ChillTokenListener {
    ...
    public void doDisable(String loginType, Object loginId, String service, int level, long disableTime) {
        log.info("账号 {} [{}服务] 被封禁 (loginType={}), 封禁等级={}, 解封时间为 {}", loginId, loginType, service, level, ChillFoxUtil.formatAfterDate(disableTime * 1000));
    }
     ...
     */
    @Override
    public void doSetStpLogic(AuthLoginLogic stpLogic) {
        if (stpLogic != null) {
            log.info("会话组件 AuthLoginLogic(type={}) 重置成功: {}", stpLogic.getLoginType(), stpLogic.getClass());
        }
    }
     ...
     */
    @Override
    public void doSetConfig(ChillTokenConfig config) {
        if (config != null) {

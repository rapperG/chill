...
/**
 * Chill-Token 持久层接口，默认实现类（基于内存 Map，系统重启后数据丢失）
 *
 * @author chill
 * @since 1.0
 */
public class ChillTokenDaoDefaultImpl implements ChillTokenDao {
     ...
     * 构造函数
     */
    public ChillTokenDaoDefaultImpl() {
        initRefreshThread();
    }
    ...
    public void set(String key, String value, long timeout) {
        if (timeout == 0 || timeout <= ChillTokenDao.NOT_VALUE_EXPIRE) {
            return;
        }
        dataMap.put(key, value);
        expireMap.put(key, (timeout == ChillTokenDao.NEVER_EXPIRE) ? (ChillTokenDao.NEVER_EXPIRE) : (System.currentTimeMillis() + timeout * 1000));
    }

    @Override
    public void update(String key, String value) {
        if (getKeyTimeout(key) == ChillTokenDao.NOT_VALUE_EXPIRE) {
            return;
        }
        dataMap.put(key, value);
    }
    ...
    public List<String> searchData(String prefix, String keyword, int start, int size, boolean sortType) {
        return ChillFoxUtil.searchList(expireMap.keySet(), prefix, keyword, start, size, sortType);
    }
    ...
    void clearKeyByTimeout(String key) {
        ...
        // 		2、不是 [ 永不过期 ]。
        // 		3、已经超过过期时间。
        if (expirationTime != null && expirationTime != ChillTokenDao.NEVER_EXPIRE && expirationTime < System.currentTimeMillis()) {
            dataMap.remove(key);
            expireMap.remove(key);
        }
    }
    ...
    long getKeyTimeout(String key) {
        ...
        if (expire == ChillTokenDao.NEVER_EXPIRE) {
    ...
    public void initRefreshThread() {
        ...
        if (ChillManager.getConfig().getDataRefreshPeriod() <= 0) {
            return;
        }
        ...
        this.refreshThread = new Thread(() -> {
            for (; ; ) {
                try {
                    ...
                    // 休眠N秒
                    int dataRefreshPeriod = ChillManager.getConfig().getDataRefreshPeriod();
